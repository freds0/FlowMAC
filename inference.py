...
from matcha.utils.model import normalize, denormalize, fix_len_compatibility

# HiFiGAN import
try:
    from matcha.hifigan.models import Generator as HiFiGANGenerator
...
def load_hifigan(checkpoint_path, config_path, device):
    ...
    if not HAS_LOCAL_HIFIGAN:
        raise ImportError("HiFiGAN modules not found in matcha/hifigan.")
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, required=True, help="FlowMAC Checkpoint (.ckpt)")
    parser.add_argument("--audio", type=str, required=True, help="Input audio (.wav)")
    parser.add_argument("--vocoder_ckpt", type=str, default=None, help="HiFiGAN Checkpoint path")
    parser.add_argument("--vocoder_config", type=str, default="matcha/hifigan/config_v1.json", help="HiFiGAN Config JSON")
    ...
    wav = load_audio(args.audio, target_sr=22050).to(args.device)
    ...
    # The Matcha/FlowMAC standard uses 2 downsamplings (factor of 4 total) or
    # basic compatibility. We use the repo's helper function.
    ...
    if vocoder is not None:
        ...
        sf.write(out_path, wav_numpy, 22050)
